...
class FILTRE():
    ...
    def Forward_update(self,mode=None,f_low=None,f_high=None,sample_rate=None):
        if mode != None:
    ...
    def Feedback_update(self,mode=None,f_low=None,f_high=None,sample_rate=None):
        if mode != None:
    ...
    def Forward_Current_Taps(self):
        if self.FORWARD_mode == 'PASSE-BAS':
            return firdes.low_pass(self.FORWARD_GAIN,self.FORWARD_SAMPLE_RATE,self.FORWARD_F_LOW,self.FORWARD_F_TRANSI,self.FORWARD_WINDOW,self.FORWARD_BETA)
            ...
            if 'taps' in txt:
                ...
                #print(out)
                def resampler(list:list):
                    out = []
                    size = round(list.__len__()/4)
                    for i in range(size,size*3,1):
                        out.append(list[i])
                    #print(out)
                    return out
                return resampler(out)
        
    def Feedback_Current_Taps(self):
        if self.FEEDBACK_mode == 'PASSE-BAS':
            return firdes.low_pass(self.FEEDBACK_GAIN,self.FEEDBACK_SAMPLE_RATE,self.FEEDBACK_F_LOW,self.FEEDBACK_F_TRANSI,self.FEEDBACK_WINDOW,self.FEEDBACK_BETA)
    ...
    def Forward_plot_init(self):
        ...
        self.FORWARD_fig.subplots_adjust(top=0.95, bottom=0.13, left=0.1, hspace=1.2,right=0.975)
    ...
    def Feedback_plot_init(self):
        ...
        self.FEEDBACK_fig.subplots_adjust(top=0.95, bottom=0.13, left=0.1, hspace=1.2,right=0.975)
